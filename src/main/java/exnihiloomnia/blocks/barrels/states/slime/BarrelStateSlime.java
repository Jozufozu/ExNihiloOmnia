...
public class BarrelStateSlime extends BarrelState {
	...
	public String[] getWailaBody(TileEntityBarrel barrel) {
		if (barrel.getTimerStatus() >= 0 && barrel.getTimerStatus() < 1.0d ) {
			description[0] = I18n.format("exnihiloomnia.info.slime.summoning") + " " + I18n.format("entity.Slime.name") + String.format("%.0f", barrel.getTimerStatus() * 100) + "%";
			return description;
		}
	...
	public void provideInformation(TileEntityBarrel barrel, ProbeMode mode, IProbeInfo info, EntityPlayer player, World world, IBlockState blockState, IProbeHitData data) {
		...
		if (barrel.getTimerStatus() >= 0 && barrel.getTimerStatus() < 1.0d ) {
			info.text(I18n.format("exnihiloomnia.info.slime.summoning") + " " + I18n.format("entity.Slime.name"));
			info.progress((int) (barrel.getTimerStatus() * 100), 100, info.defaultProgressStyle().filledColor(0xff33ff22).alternateFilledColor(0xff33ff22));
		}
		else {
