...
object RenderUtil {
    ...
    fun renderContents(texture: TextureAtlasSprite, height: Double, color: Color) {
        ...
        tessellator.buffer.apply {
            ...
            pos(1.0, height, 1.0).tex(maxU, maxV).color(color.r, color.g, color.b, color.a).endVertex()
            pos(1.0, height, 0.0).tex(maxU, minV).color(color.r, color.g, color.b, color.a).endVertex()
            pos(0.0, height, 0.0).tex(minU, minV).color(color.r, color.g, color.b, color.a).endVertex()
            pos(0.0, height, 1.0).tex(minU, maxV).color(color.r, color.g, color.b, color.a).endVertex()
        }
    ...
    private fun renderTexturedQuad(renderer: BufferBuilder, texture: TextureAtlasSprite, vertices: Array<Vec3d>, contentHeight: Double, r: Float, g: Float, b: Float, a: Float) {
        val minU = texture.minU.toDouble()
        val maxU = texture.maxU.toDouble()
        val minV = texture.getInterpolatedV(texture.iconHeight - texture.iconHeight * contentHeight).toDouble()
        val maxV = texture.maxV.toDouble()
